...
public class Main {
	...
	public static void addWeapons(ArrayManager h, Scanner sc) // after entering player name this is being called
	{
		...
		while (weaponName.compareTo("end") != 0) { // this will run until user enters "end"
			...
			else if (h.get(w.weaponName).item.weaponName == weaponName) {
				System.out.println("The weapon " + h.get(w.weaponName).item.weaponName + " already exists");
				h.get(w.weaponName).numberInStock += getInteger(sc,
						"***Please enter the amount of this item you would like to add: => ");
	...
	public static void deleteWeapon(ArrayManager h, Scanner sc, Player p1) {
		...
		while (choice.compareTo("end") != 0 && !p.inventoryFull()) {
			...
			if (si != null && si.item.weaponName != "DELETED") {
				if (p.money >= si.item.cost) { // checking if the player has enough coins ()
					if (p.backpack.currWeight + si.item.weight <= 90 && p.numItems + 1 <= 30) {
